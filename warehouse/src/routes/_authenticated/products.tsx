...
import { Plus, Search, ScanLine, Pencil, Trash2, ImagePlus, ImageIcon, Calendar, User as UserIcon, Barcode, FolderTree, ChevronRight, ChevronDown, Maximize2, Minimize2, PackageCheck, AlertTriangle, PackageX, LayoutGrid, Zap, Check, SkipForward, Warehouse, Upload, FileJson } from "lucide-react";
...
function ProductsPage() {
  ...
  const onImportFilePick = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!Array.isArray(rows)) { toast.error("Invalid file: expected a products array"); return; }
  ...
  const runImport = async () => {
    ...
    try {
      ...
      (categories as any[]).forEach((c: any) => { catMap[c.name.toLowerCase()] = c.id; });
      ...
      for (const row of importPreview) {
        ...
        // Resolve category_id from category or sub_category
        const catName = (row.sub_category || row.category || "").trim().toLowerCase();
        ...
        const payload: Record<string, any> = {
          ...
          size: row.size || null,
          unit: row.unit || row.size || null,
          barcode: row.barcode || null,
          image_url: row.image_url || null,
          price: row.price ?? row.price_unit_jpy ?? 0,
          price_10: row.price_10 ?? row.price_10_jpy ?? null,
          price_case: row.price_case ?? row.price_case_jpy ?? null,
          pcs_per_case: row.pcs_per_case ?? null,
          stock: row.stock ?? 0,
          low_stock_threshold: row.low_stock_threshold ?? 5,
          category_id,
        };

        // Upsert by SKU (skip if no SKU)
        const { error } = sku
          ? await supabase.from("products").upsert(payload, { onConflict: "sku", ignoreDuplicates: false })
          : await supabase.from("products").insert(payload);

        if (error) {
          if (error.code === "23505") { skipped++; } // unique violation → already exists
          else { console.error(error); failed++; }
        } else {
          created++;
        }
      }
      ...
      qc.invalidateQueries({ queryKey: ["products"] });
      toast.success(`Import done: ${created} added/updated, ${skipped} skipped, ${failed} failed`);
      setImportOpen(false);
  ...
  return (
    <div className="p-6 md:p-10 max-w-7xl mx-auto">
      <PageHeader
        ...
        actions={canEdit ? (
          <div className="flex gap-2 flex-wrap items-center">
            ...
            <label className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm font-medium bg-secondary hover:bg-secondary/80 cursor-pointer border border-input transition-colors">
              <Upload className="size-4" /> Import from Catalog
              <input type="file" accept=".json,application/json" className="hidden" onChange={onImportFilePick} />
            </label>
      ...
      <Dialog open={importOpen} onOpenChange={(v) => { if (!v) { setImportOpen(false); setImportPreview(null); setImportFile(null); } }}>
        <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2"><FileJson className="size-5" /> Import from Catalog</DialogTitle>
          </DialogHeader>
          {importPreview && (
            <div className="flex-1 overflow-hidden flex flex-col gap-3 min-h-0">
              <p className="text-sm text-muted-foreground">{importPreview.length} products found in <span className="font-medium text-foreground">{importFile?.name}</span>. Existing products with same SKU will be updated.</p>
              <div className="flex-1 overflow-y-auto border rounded-lg divide-y text-sm min-h-0">
                {importPreview.slice(0, 200).map((row, i) => (
                  <div key={i} className="flex items-center gap-3 px-3 py-2">
                    {row.image_url && <img src={row.image_url} alt="" className="w-10 h-10 rounded object-cover shrink-0 bg-muted" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />}
                    {!row.image_url && <div className="w-10 h-10 rounded bg-muted shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="font-medium truncate">{row.name}</div>
                      <div className="text-xs text-muted-foreground truncate">{[row.sku, row.brand, row.size].filter(Boolean).join(" · ")}</div>
                    </div>
                ...
                {importPreview.length > 200 && <div className="px-3 py-2 text-xs text-muted-foreground text-center">…and {importPreview.length - 200} more</div>}
              </div>
            </div>
          )}
          <DialogFooter>
            <Button variant="ghost" onClick={() => { setImportOpen(false); setImportPreview(null); setImportFile(null); }}>Cancel</Button>
            <Button className="gradient-primary text-primary-foreground border-0" disabled={importing} onClick={runImport}>
              {importing ? "Importing…" : `Import ${importPreview?.length ?? 0} products`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
