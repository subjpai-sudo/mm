...
async function processOne(p) {
  ...
  // Skip Lovable gateway (402 credits exhausted) — go straight to Google with paid key
  const g = await genGoogleDirect(prompt, ref);
  if (!g.ok) throw new Error(`gen fail: ${g.status} ${g.err || ""}`);
  // Store data URL directly in DB (matches existing pattern; storage bucket isn't accessible from Node)
  await update(dbRef(db, "catalog/images"), { [String(p.no)]: g.url });
  return g.url;
}
